...
| Date | Change |
|---|---|
| 2026-09-22 | **The Render API service has failed every deploy since PR #10, and is currently down (502 on every route).** `aluminium-store-api` failed on `8a2550ae`, `32073efd`, `fbbd0e8c`, `ee08fbbe` and `2d725ebf` — every recorded deploy of the stack — while the storefront deployed cleanly each time and is still serving its last good build. `preDeployCommand` runs `prisma migrate deploy && npm run prisma:seed`, and the seed aborts under `NODE_ENV=production` without `SEED_ADMIN_PASSWORD`/`SEED_TRADE_PASSWORD`; a `preDeployCommand` failure rolls the release back before the new build is promoted, so the API never becomes healthy. Two separate defects: (1) the guard sat ~300 lines into `main`, *after* the catalogue had been written, so a deploy short a secret died against a half-seeded database — products present, clearance lines, CMS copy and merchandising rows absent — which is why the failure reads as a catalogue bug rather than a config one; (2) the guard was unconditional, but both user upserts are `update: {}` so the password is never read once the account exists, meaning a redeploy of an already-seeded database was blocked over a secret it would not use. The guard now runs before the first write, checks the two rows, and requires a password only for an account that is absent; the password is read on the create path and its bcrypt hash computed there. Verified against a scratch database: fresh without secrets aborts with 0 products written; fresh with secrets completes (2,147 products, 4 clearance lines, 3 deals, 10 pairings, 3 collections, 1 admin); already-seeded without secrets now completes; dev mode still prints the dev credentials. bcrypt comparison confirms the supplied production passwords hash into the seeded accounts and the published dev defaults do not. 131 backend tests pass across 14 suites (4 new pinning the guard's position and its per-account conditionality). **Outstanding and not fixable from the repository:** the two secrets must be set in the Render dashboard (they are `sync: false` and deliberately uncommitted — the dev defaults are published in this public repo). The database's current contents cannot be inspected from here, so whether the account rows exist is unconfirmed; if the database is genuinely fresh, both secrets are still needed for the seed to populate it. See `docs/35-deployment-render.md`. |
| 2026-09-22 | **Added the ten merchandising sections and closed the Clearance CTA defect.** The homepage gained Best Sellers, Top Rated, Shop by Finish, Recently Viewed, Recommended, Daily Deals, Featured Products, Collections and Budget Shop; the PDP gained Product Pairings plus Recently Viewed / Recommended. Each section states its basis in the UI ("cumulative units sold", "sub-category of the last product you viewed") rather than implying a personalisation engine, and each either renders an honest empty state or — for Clearance, Recently Viewed and Recommended — renders nothing at all, because an empty shelf with invented content is worse than no shelf. **The Clearance defect is fixed:** the hero's "Shop Clearance" CTA pointed at `/#clearance`, an anchor that only exists while the carousel is non-empty and is replaced whenever CMS hero slides are present, so the link could dangle; it now targets `/catalogue?clearance=1`, which queries `GET /catalog/clearance` directly and survives the carousel being hidden. Hero copy claiming "up to 20% off ... Bronze Anodised" was also wrong against the seeded data (deepest markdown is 35%, no Bronze Anodised lines are on clearance) and now reads "up to 35% ... windows, doors and hardware". Two follow-on defects of the same class were found and fixed while wiring the sections: `/catalogue?finish=<id>` was accepted but ignored, so every Shop by Finish swatch landed on the unfiltered catalogue under a filtered heading, and `/collections/<slug>` — the target of every Collection row's "View all N" link — did not exist and 404'd. Added `finishId` to `CatalogQuery` with the predicate applied to both the page and the count so the swatch total matches the grid, added the `/collections/[slug]` route with a real 404 via `notFound()`, and added an app-wide `not-found.tsx`. **Also fixed a dead code path in the signed-in history:** `recently-viewed`, `recommended` and `product-view` declared `req.user` but carried no guard, so `userId` was always null and a signed-in buyer's history could never follow their account across devices. They now carry a new `OptionalJwtAuthGuard` — a guest is still served keyed by session id, an invalid token is treated as "not signed in" rather than a 401, and the account path actually runs. Verified against Postgres: guest history is recency-ordered and strictly scoped to its own session, a signed-in write is recorded against the account and is invisible to any session id, and the recommendation reason names the real anchor ("More from Casement Windows — the sub-category of the last product you viewed"). The seed's merchandising summary reported rows *created*, so a second idempotent run printed `0 pairings` while ten existed and read as a failed seed; it now reports table totals with the new count alongside. 127 backend tests pass across 14 suites (5 new for the finish filter); frontend builds clean and every section and link checked on the running stack. Spec written up in `docs/37-merchandising-sections.md`. |
